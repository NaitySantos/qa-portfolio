...
Acessar site e escolher um filme ativo comercialmente.
Selecionar o assento de escolha no mapa interativo.
Clicar no tipo de entrada "Meia Estudante" do catálogo e preencher o código da carteirinha.
Preencher o cartão financeiro de crédito válido.
Acionar o botão "Finalizar Compra".
...
Passos:
Acessar tela de check-out com ingresso de estudante.
...
email = driver.find_element(by="id", value="email") email.send_keys("nataly.qa@cine.com")

button = driver.find_element(by="id", value="btn-login") button.click()

4. Validando resultado esperado assertivo
assert "Painel de Ingresso" in driver.title driver.quit() ```
